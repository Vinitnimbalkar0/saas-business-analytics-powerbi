  ...
  BEGIN
	DECLARE 
		...
		BEGIN TRY
			...
			PRINT'===================================================='
			PRINT'Loading SaaS Silver Layer'
			PRINT'===================================================='
		 ...
		 PRINT '------------------------------------------------------------'
			...
			-----------------------------------------
			SET @start_time = GETDATE();
			PRINT'Truncating Table : silver.subscriptions'
		 ...
		 PRINT '------------------------------------------------------------'
			...
			PRINT'------------------------------------------------'
			PRINT'-----------Loading Churn Events Table----------'
			PRINT'------------------------------------------------'
			...
			-----------------------------------------
			SET @start_time = GETDATE();
			PRINT'Truncating Table : silver.churn_events'
			TRUNCATE TABLE silver.churn_events;	
		 ...
		 PRINT '------------------------------------------------------------'
			...
			PRINT'------------------------------------------------'
			PRINT'----------Loading Support Tickets Table--------'
			PRINT'------------------------------------------------'
			...
			-----------------------------------------
			SET @start_time = GETDATE();
			PRINT'Truncating Table : silver.support_tickets'
          ...
          INSERT INTO silver.support_tickets
            ...
            (
                SELECT
                    ...
                    TRY_CONVERT(DECIMAL(10,2), resolution_time_hours) AS resolution_time_hours,

                    TRIM(priority) AS priority,

                    TRY_CONVERT(INT, first_response_time_minutes) AS first_response_time_minutes,

                    TRY_CONVERT(TINYINT, satisfaction_score) AS satisfaction_score,

                    CASE
                        WHEN UPPER(TRIM(escalation_flag)) = 'TRUE' THEN 1
                        WHEN UPPER(TRIM(escalation_flag)) = 'FALSE' THEN 0
                        ELSE NULL
                    ...
                    (
                        ...
                        ORDER BY TRY_CONVERT(DATE, submitted_at, 23) DESC
                    ) AS flag_last
		 ...
		 PRINT '------------------------------------------------------------'
			...
			PRINT'------------------------------------------------'
			PRINT'----------Loading Feature Usage Table--------'
			PRINT'------------------------------------------------'
			...
			-----------------------------------------
			SET @start_time = GETDATE();
			PRINT'Truncating Table : silver.feature_usage'
			TRUNCATE TABLE silver.feature_usage;	
			PRINT'Inserting Data Into : bronze.feature_usage '
            INSERT INTO silver.feature_usage
            (
                usage_id,
                subscription_id,
                usage_date,
            ...
            SELECT
	            ...
	            CASE 
		            ...
		            ELSE 'High'
	            END AS usage_count_band,
	            CASE
		            WHEN usage_duration_secs < 300 THEN 'Short'
		            WHEN usage_duration_secs < 1000 THEN 'Medium'
		            ELSE 'Long'
	            END AS usage_duration_band,
	            CASE
		            WHEN error_count = 0 THEN 'No Errors'
		            ELSE 'Has Errors'
	            END AS error_status,
	            CASE
		            WHEN is_beta_feature = 1 THEN 'Beta'
		            ELSE 'Stable'
            ...
            SELECT 
	            usage_id,
	            subscription_id,
	            TRY_CONVERT(DATE,usage_date,23) AS usage_date,
	            TRIM(feature_name) as feature_name,
	            usage_count,
	            usage_duration_secs,
	            error_count,
	            CASE
		            WHEN UPPER(TRIM(is_beta_feature)) = 'TRUE' THEN 1
		            WHEN UPPER(TRIM(is_beta_feature)) = 'FALSE' THEN 0
		            ELSE NULL
	            END AS is_beta_feature,
	            ROW_NUMBER() OVER(PARTITION BY usage_id ORDER BY TRY_CONVERT(DATE,usage_date,23) DESC ) AS flag_test
            FROM bronze.feature_usage
            ) t
            WHERE flag_test = 1
        ...
        PRINT '================================================';
        PRINT 'SaaS Silver Layer Load Completed';
